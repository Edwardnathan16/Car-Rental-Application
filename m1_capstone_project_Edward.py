...
car_data = {
    "BR01MT" : {"Tipe" : "Brio", "Transmisi" : "MT", "Seats" : 5, "Harga" : 250000},
    "AV01MT" : {"Tipe" : "Avanza", "Transmisi" : "MT", "Seats" : 6, "Harga" : 300000},
    "IN01AT" : {"Tipe" : "Innova", "Transmisi" : "AT", "Seats" : 8, "Harga" : 500000},
    "HI01MT" : {"Tipe" : "Hiace", "Transmisi" : "MT", "Seats" : 12, "Harga" : 700000}
}

column_name = ["Id", "Tipe", "Transmisi", "Seats", "Harga"]
...
def menu1_sub1():
    ...
    for i in car_data.keys():
        print(f"{i: <10}|{(car_data[i]['Tipe']): <20}|{(car_data[i]['Transmisi']): <10}|{str(car_data[i]['Seats']): <10}|{str(car_data[i]['Harga']): <10}")
    print()
...
def menu1_sub2():
    ...
    if user_input == '1':
        ...
        check_input_value("Tipe")
    elif user_input == '3':
        check_input_value("Transmisi")
    elif user_input == '4':
        check_input_value("Seats")
    elif user_input == '5':
        check_input_value("Harga")
    elif user_input == '6':
        menu_1()
    else:
...
def check_input_value(input_column_name):
    ...
    if input_column_name == "Id":
    ...
    elif input_column_name == "Transmisi":
        user_input = input(f"Masukkan {input_column_name} Mobil dari mobil yang ingin ditampilkan: ").upper()
        ...
        while user_input.isalpha():
            print("(INFO) Input yang anda masukkan harus berupa integer harga\n")
            user_input = input(f"Masukkan {input_column_name} Mobil dari mobil yang ingin ditampilkan: ")
...
def menu1_sub2_print(user_input, input_column_name):
    ...
    if input_column_name == "Id":
        print(f"{column_name[0]: <10}|{column_name[1]: <20}|{column_name[2]: <10}|{column_name[3]: <10}|{column_name[4]: <10}")
        print("----------+--------------------+----------+----------+----------")
        print(f"{user_input: <10}|{(car_data[user_input]['Tipe']): <20}|{(car_data[user_input]['Transmisi']): <10}|{str(car_data[user_input]['Seats']): <10}|{str(car_data[user_input]['Harga']): <10}")
...
def input_car_type():
    car_type = input("Masukkan Tipe Mobil: ")

    if car_type.isalpha():
        return car_type.capitalize()
    else:
        print("\n(INFO) Format tipe mobil yang anda masukkan salah\n")
...
def input_car_transmission():
    car_transmission = input("Masukkan Tipe Transmisi Mobil (MT/AT): ").upper()
    ...
    if len(car_transmission) == 2:
        if car_transmission[0] == 'M' or car_transmission[0] == 'A' and car_transmission[1] == 'T':
            ...
            print("\n(INFO) Format transmisi yang anda masukkan salah\n")
...
def input_car_price():
    car_price = input("Masukkan Harga Rental dari Mobil: ")

    if car_price.isnumeric():
        if int(car_price) > 100000 and int(car_price) < 5000000:
            return int(car_price)
        else:
            print("\n(INFO) Harga Rental Mobil yang Anda Masukkan Salah\n")
            return input_car_price() 
    else:
        print("\n(INFO) Format Harga Rental Mobil yang Anda Masukkan Salah\n")
        return input_car_price()
...
def create_a_car_item():
    ...
    if user_input == 'ya':
        ...
        while new_car_type[0:2].upper() != new_car_id[0:2]:
            print("\n(INFO) Input Tipe yang anda masukkan harus sesuai dengan input Tipe di ID Anda\n")
            new_car_type = input_car_type()
        ...
        while new_car_transmission.upper() != new_car_id[4:]:
            print("\n(INFO) Input transmisi yang anda masukkan harus sesuai dengan input Transmisi di ID Anda\n")
            new_car_transmission = input_car_transmission()
...
def yes_no_insert(new_car_id, new_car_type, new_car_transmission, new_car_seats, new_car_price):
    user_input = input("Apakah anda ingin memasukkan mobil ke dalam tabel(y/n)? ").lower()

    # Memastikan apakah user ingin menginsert item ke dalam tabel
    if user_input == 'y':
        car_data[new_car_id] = {"Tipe" : new_car_type, "Transmisi" : new_car_transmission, "Seats" : new_car_seats, "Harga" : new_car_price}
        menu1_sub1()
    elif user_input == 'n':
        menu1_sub1()
        create_a_car_item()
...
def update_car_info(car_id, column_name):
    ...
    yes_no = input("Apakah anda ingin mengupdate data dari data mobil (y/n)? ")
...
def continue_or_no():
    user_input = input("Apakah anda ingin mengupdate informasi dari mobil lagi (y/n)?").lower()
...
def yes_no_delete(column_name, input_value):
    user_input = input("\nApakah anda yakin ingin menghapus record ini (y/n)?")
...
def menu_5():
    exit_input = input("Apakah anda ingin keluar dari program (y/n)? ").lower()
    if exit_input == 'y':
        exit()
    elif exit_input == 'n':
        main_menu()
    else:
        print("\n(INFO) Tolong masukkan 'y' atau 'n'")
        menu_5()
...
def main_menu():
    print()
    print("Selamat Datang di EDWARD GARAGE\nSilahkan pilih menu di bawah")
    print('''
    List Menu:
